...
export const DisplayHotels = () => {
  ...
  const handlePriceRangeChange = (event) => {
    ...
    if (priceRangeFilters.includes(value)) {
      setPriceRangeFilters(priceRangeFilters.filter((filter) => filter !== value));
    } else {
  ...
  const handlePropertyTypeChange = (event) => {
    ...
    if (propertyTypeFilters.includes(value)) {
      setPropertyTypeFilters(propertyTypeFilters.filter((filter) => filter !== value));
    } else {
  ...
  return (
    ...
    <div className="" style={{ maxHeight: '100%', display:'flex' , background:'rgb(180,180,180)',padding:'50px'}}>
      ...
      <Form onSubmit={handleSubmit} style={{marginLeft:'30px'}}>
        ...
        <div className="filter-section" style={{marginTop:'20px',display:'flex' ,flexDirection:'column', justifyContent:'space-between',height:'350px'}}>
    ...
    <div> 
         ...
         <DisplayHotelList  data={filteredData.length > 0 ? filteredData : data}/>
      </div>
      </div>
    </div>
    </>
